#' plotGCaMP_multi
#'
#' Function is a wrapper for expfit.all which outputs corrected GCaMP signals as well as plots showing
...
#' @param linear optional argument piped into expfit.all include a linear term in the fit?
...
plotGCaMP_multi <- function(FileFilter,
  ...
  if (matlab == TRUE) {
    ...
    data <- purrr::map(files, ~ expfit.all(
      filename = .,
      skip.time = 10,
  ...
  # check for same length video files:
  Check2 <- ArgumentCheck::newArgCheck()

  if (data %>% unnest(cols = c(data)) %>% group_by(animal) %>% tally() %$% unique(n) %>% length() != 1) {
    ArgumentCheck::addError(
      msg = print(c(
        "One or more of your video files are of different length, check these files:",
  ...
  if (use.Fmax == TRUE) {
    ggsave(plots,
      filename = file.path(folderPath, paste0(genotype, "_", cue, neuron, "_delFmaxplots.png")),
      width = 11, height = 8.5, units = "in"
    )
  } else {
    ggsave(plots,
      filename = file.path(folderPath, paste0(genotype, "_", cue, "_", neuron, "_", food, "_plots.png")),
      width = 11, height = 8.5, units = "in"
    )
  }
